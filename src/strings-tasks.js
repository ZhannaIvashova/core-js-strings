...
function sumOfCodes(str) {
  ...
  const sumStr = arrStr.reduce((acc, char) => acc + char.charCodeAt(0), 0);
  return sumStr;
}
 ...
 */
function containsSubstring(str, substrin) {
  return str.includes(substrin);
}
...
function countVowels(str) {
  ...
  const result = str.split('').filter((char) => vowels.includes(char));
...
function reverseWords(str) {
  ...
  const resulrArr = [];
  reverseStr.forEach((element) => {
    const reverseWord = element.split('').reverse().join('');
    resulrArr.push(reverseWord);
  });
